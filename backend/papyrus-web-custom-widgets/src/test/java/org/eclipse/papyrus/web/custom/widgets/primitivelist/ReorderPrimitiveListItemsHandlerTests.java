...
import org.eclipse.papyrus.web.custom.widgets.primitivelist.handler.ReorderPrimitiveListItemsEventHandler;
...
/**
 * Unit tests for the reorder primitive list items handler.
 *
 * @author Jerome Gout
 */
public class ReorderPrimitiveListItemsHandlerTests {
    ...
    @Test
    public void testReorderdPrimitiveListItemsHandler() {
        String itemId = "Item Id";
        String changeKind = ChangeKind.SEMANTIC_CHANGE;

        var input = new ReorderPrimitiveListItemsInput(UUID.randomUUID(), FORM_ID.toString(), UUID.randomUUID()
                .toString(), WIDGET_ID, itemId, 0, 1);

        Map<String, Object> parameters = new HashMap<>();
        parameters.put(CHANGE_DESCRIPTION_PARAMETER_KEY, itemId);
        AtomicBoolean hasBeenExecuted = new AtomicBoolean();
        Function<ReorderPrimitiveListHandlerParameters, IStatus> reorderHandler = params -> {
        ...
        var handler = new ReorderPrimitiveListItemsEventHandler(formQueryService, new ICollaborativeFormMessageService.NoOp(), new SimpleMeterRegistry());
    ...
    @Test
    public void testReorderPrimitiveListItemsHandlerReadOnly() {
        String itemId = "Item Id";
        String changeKind = ChangeKind.SEMANTIC_CHANGE;
