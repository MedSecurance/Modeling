...
/**
 * Provides the behavior on a graphical drop event in the "Deployment" Diagram.
 *
 * @author <a href="mailto:jessy.mallet@obeo.fr">Jessy Mallet</a>
 */
public class DeploymentGraphicalDropBehaviorProvider implements IWebInternalSourceToRepresentationDropBehaviorProvider {
     ...
     *            Logger used to report errors and warnings to the user
     */
    public DeploymentGraphicalDropBehaviorProvider(IEditingContext editionContext, IViewHelper viewHelper, IObjectService objectService, ECrossReferenceAdapter crossRef,
    ...
    public void handleGraphicalDrop(EObject droppedElement, EObject targetElement, Node droppedNode, Node targetNode) {
        ...
        new GraphicalDropSwitch(optionalTargetNode, optionalOldContainer, optionalNewContainer, this.viewHelper, this.diagramNavigator, droppedNode, this.logger) //
                .withDropChecker(new DeploymentInternalSourceToRepresentationDropChecker()) //
                .withDropProvider(new DeploymentInternalSourceToRepresentationDropBehaviorProvider()) //
